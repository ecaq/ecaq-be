...
public static class AboutEndpoint
{
    ...
    {
        var group = routes.MapGroup("/api/about").WithTags(nameof(AboutModel));

        group.MapGet("/", async (AppDbContext db) =>
        {
            return await db.AboutModels.ToListAsync();
        })
        .WithName("GetAbouts")
        .WithOpenApi();

        group.MapGet("/{id}", async Task<Results<Ok<AboutModel>, NotFound>> (int id, AppDbContext db) =>
        {
            return await db.AboutModels.AsNoTracking()
